...
def train(args):
    ...
    data_test_loader = DataLoader(Dataset(height=height, width=width,
            ...
            video_dir=os.path.join(args.test_set_dir, 'videos/test')), batch_size=batch_size)
    ...
    for epoch in range(start_epoch, n_epoch):
        ...
        for batch_n, data in enumerate(data_loader, 0):
            ...
            if (batch_n + 1) % print_loss_freq == 0:
                print('batch: {:<10}   |   loss: {:<10}    |    average time per batch: {:<20}'.format(batch_n + 1, np.array(losses).mean(), (time.time() - start_time) / (batch_n + 1)))
                print(g_model.weight, file=open('log.txt', 'w'))
