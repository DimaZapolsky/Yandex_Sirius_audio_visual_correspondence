...
def train(args):
    ...
    if args.optimizer.lower() == 'adam':
        ...
        opt_cls = optim.SGD
        opt_kwargs = {'momentum': 0.9, 'weight_decay': 1e-5}
    else:
    ...
    for epoch in range(start_epoch, n_epoch):
        ...
        for batch_n, data in enumerate(data_loader, 0):
            ...
            for i in range(n_video):
                ...
                loss = F.binary_cross_entropy(g_res.squeeze(1), (data[1][:, i, :].to(device).squeeze(1) > (audio_sum.squeeze(1) / n_video)).type(torch.Tensor).to(device), weight=weight.to(device)).to(device)
