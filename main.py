    ...
    for batch_n, data in enumerate(dataloader, 0):
        ...
        if (batch_n % example_freq == 0):
            with torch.no_grad():
                ...
                full_sound = np.repeat(located_sound_picture, 16, axis=1)
                full_sound = np.repeat(full_sound, 16, axis=2)
                output = torch.permute(located_sound_picture * 0.2 + picture.numpy() * 0.8)
